...
function drawText(dParams, file, trials)
  ...
  t2H(1) = text(0.00, 0.45, {'Trials:', 'Correct:', 'Failed:', 'Early:'});
  t2H(2) = text(0.35, 0.45, {sprintf('%d', file.trials), sprintf('%d', numCorrect), sprintf('%d', numFailed), ...
                      sprintf('%d', numEarly)});
  t2H(3) = text(0.60, 0.45, {' ', sprintf('%.0f%%', numCorrect / totalTrials * 100.0), ...
...
function oKernel(~, file, trials, plotLastOStim)
  ...
  if plotLastOStim
    doOneKernelPlot(9, trialProfile, 'stim', startTimeMS, endTimeMS, 'Optical Stim Last Trial', 'Power (mW)', 0, 0);
  end
...
function powerVTrial(dParams, ~, trials, indices)
  ...
  yyaxis right
  set(gca, 'yColor', 'black');
  plot(indices.correct(2:end), diff(indices.correct), 'color', [0.8500, 0.3250, 0.0980]);
