function [fileProfilesFast, fileProfilesSlow] = getOptoStimFastSlowMiss(U, dataDirName)
...
%   meanPower = meanPower(firstStimIndex:lastStimIndex);
  ...
  % get the counts for stimulated trials
	[~, hitIndices, missIndices, ~] = countsAndIndices(true, row, file, trials, eotCodes, stimIndices);
  ...
  fileProfilesFast = [normProfiles(profiles); -missProfiles];
  profiles = getStimProfiles(trials(slowHitIndices), plotStartMS, plotEndMS, true, false);
  fileProfilesSlow = [normProfiles(profiles); -missProfiles];
end
